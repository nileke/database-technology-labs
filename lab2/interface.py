...
# the built in python function raw_input to get from the user.  All are stored in a variable that we chose to call params.
        params = {'host':'localhost', 'user':raw_input("Username: "), 'database':raw_input("Database: "), 'password':raw_input("Password: ")}
...
# Here we define a member function that we can later call repeatidly
    ...
    def remove(self):
        ...
        table = raw_input("Choose (1) table: ")
        print table

        #delete_column = raw_input("Enter row you want to delete (id): ")
        #delete_tuple = raw_input("Enter id of row to delete: ")

        delete_filter = raw_input("Enter delete condition: ")
        
        try:
            # Defining a transaction to commit deletion from database
            delete_query = "DELETE FROM %s WHERE %s;" % (table, delete_filter)
        except (NameError, ValueError, TypeError, SyntaxError):
            raise
            # print "  Bad input."
            return
    ...
    def insert(self):
        ...
        # Let's user choose table to insert into
        table = raw_input("Choose (1) table: ")
        

        insert_columns = raw_input("Enter columns to insert (comma separated): ")
        insert_values = raw_input("Enter values to insert (comma separated): ")

        try:
            # Defining a transaction to commit insert to database
            insert_query = "INSERT INTO %s (%s) VALUES (%s);" % (table, insert_columns, insert_values)   
        except (NameError, ValueError, TypeError, SyntaxError):
            print "  Bad input."
            return
        ...
        # commit needed to save transaction otherwise transaction still running 
        # until deleted (program cancelled)
        self.conn.commit()
